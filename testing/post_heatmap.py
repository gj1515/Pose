...
def non_max_suppress(param, heatmaps, upsampFactor=1., bool_refine_center=True, bool_gaussian_filt=False):
    """
    ...
    win_size = 2
    num_joints = heatmaps.shape[2] - 1 # remove background
...
def decode_pose_single_person(heatmap, params):
    heatmap = heatmap.transpose(1, 2, 0) # (256, 256, 18)
    joint_list_per_joint_type = non_max_suppress(params, heatmap)
    # (x, y, heat, total_jnt)

    # assume single person
    num_joints = heatmap.shape[2] - 1
    keypoints = np.zeros(shape=(num_joints, 3), dtype=float)
    for i, joint_list in enumerate(joint_list_per_joint_type):
        cnt = joint_list.shape[0]
        if cnt == 1:
            keypoints[i, :] = joint_list[0, :-1] #(x, y, heat)
        elif cnt > 1:
            arr = joint_list[:, 2]
            max_heat = np.amax(arr, axis=0)
            index = np.where(max_heat == arr) # index = 2d
            keypoints[i, :] = joint_list[index[0][0], :-1] #(x, y, heat)
        #else: # cnt == 0
        #    person_joints.append([0, 0, 0])  # (x, y, heat)
