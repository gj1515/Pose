...
from utils.omni_utils.inference import get_max_preds
...
def save_images(batch_image, batch_joints, batch_joints_vis, output_dir, meta, iteration):
    ...
    gt_joints_vis = meta['joints_vis']

    gt_file = 'D:/Dev/Dataset/coco/annotations/person_keypoints_val2017.json'
    ...
    for i in range(batch_joints.shape[0]):
        ...
        i_gt = int(os.path.basename(filename).split('.')[0])
        ...
        for j in range(gt_joints.shape[1]):
            if gt_joints_vis[i][j][0] > 0:
                circle = mpatches.Circle((int(gt_joints[i][j][0]), int(gt_joints[i][j][1])),
                                         radius=int(h / 100),
                                         ec='black',
                                         fc=joint_colors[j],
                                         alpha=1,
        ...
        for k, link_pair in enumerate(link_pairs):
            if gt_joints_vis[i][link_pair[0]][0] > 0 and gt_joints_vis[i][link_pair[1]][0] > 0:
                line = mlines.Line2D(
                    np.array([gt_joints[i][link_pair[0]][0],
                              gt_joints[i][link_pair[1]][0]]),
                    np.array([gt_joints[i][link_pair[0]][1],
                              gt_joints[i][link_pair[1]][1]]),
                    ls='-', lw=int(h / 100), alpha=1, color=limb_colors[len(images_done) % len(limb_colors)] )
                line.set_zorder(0)
                ax.add_line(line)
        ...
        output_path = os.path.join(output_dir, 'val_images', f"{i_gt}.png")
        plt.savefig(output_path, format='png', bbox_inches='tight', dpi=100)
        plt.close()

        images_done.append(filename)
